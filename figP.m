...
[I,map]=rgb2ind(im,20);
imwrite(I,map,'obj1.gif','gif', 'Loopcount',inf,'DelayTime',0.03);%第一次必须创建！
for s=0:50:sg
delete(temp);
subplot(121);
CLR=clr(2+sign(dr(s+25)),:);
plot(x(s+1:s+50),y(s+1:s+50),'k.','markersize',1);
temp=plot(x(s+50),y(s+50),'o','LineWidth',2,'Color',CLR);
title({['coordinate time t=' num2str(t(s+51)) 's'];  ['proper time tp=' num2str(tp(s+51)) 's']},'Color',CLR)
subplot(122);
plot(tp(s+1:s+50),atidal(s+1:s+50),'k.');
frame=getframe(gcf);  
im=frame2im(frame);%制作gif文件，图像必须是index索引图像
[I,map]=rgb2ind(im,20);
imwrite(I,map,'obj1.gif','gif','WriteMode','append','DelayTime',0.04);
end